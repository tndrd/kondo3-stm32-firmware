...
"""
from head_control import get_controller, P_LIMITS, T_LIMITS
import time
import subprocess
    

control = get_controller()
control.set_pan(7500)
control.set_tilt(8000)
lastTilt=7500
lastPan=8000
control.set_speed(100, 10) #optional
...
def build_argparser():
    ...
    models.add_argument('-m_fd', type=Path, default="/home/pi/open_model_zoo/tools/model_tools/intel/face-detection-retail-0004/FP16/face-detection-retail-0004.xml",
                        help='Required. Path to an .xml file with Face Detection model.')
    models.add_argument('-m_lm', type=Path, default="/home/pi/open_model_zoo/tools/model_tools/intel/landmarks-regression-retail-0009/FP16/landmarks-regression-retail-0009.xml",
                        help='Required. Path to an .xml file with Facial Landmarks Detection model.')
    models.add_argument('-m_reid', type=Path, default="/home/pi/open_model_zoo/tools/model_tools/intel/face-reidentification-retail-0095/FP16/face-reidentification-retail-0095.xml",
...
def draw_detections(frame, frame_processor, detections, output_transform):
    ...
    for roi in detections: #, landmarks, identity in zip(*detections):
        ...
        if roi.size[0]*roi.size[1]>15000:
            print(roi.size[0] * roi.size[1])
            Tilt = lastTilt + int(240-((ymin+ymax)/2))
            Pan = lastPan - int(320-((xmin+xmax)/2))
        
            Tilt = control._limit(Tilt, T_LIMITS)
            Pan  = control._limit(Pan, P_LIMITS)
        
            control.set_pan(Pan)
            print(Pan, Tilt)
            control.set_tilt(Tilt)
            time.sleep(0.1)
        # control.add_tilt(Tilt)
            ...
            if voiceFlag==0:
                subprocess.Popen(['mpg123', 'Desktop/minion-hello-sound-effect.mp3'],stdin=subprocess.PIPE, stdout=None,stderr=None)
                voiceFlag=1
...
def main():
    ...
    # out = os.system("libcamera-still -t 5000")
    # print(type(out))
